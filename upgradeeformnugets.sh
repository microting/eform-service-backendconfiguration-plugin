...
if (( "$GIT_STATUS" > 0 )); then
  ...
  CURRENT_NUMBER_OF_COMMITS=`git log --oneline | wc -l`
	...
	cd ..
	cd ServiceBackendConfigurationPlugin.Integration.Test
	PACKAGES=('Microsoft.NET.Test.Sdk' 'NUnit' 'NUnit3TestAdapter' 'NUnit.Analyzers')
	PROJECT_NAME='ServiceBackendConfigurationPlugin.Integration.Test.csproj'
	REPOSITORY='eform-service-backendconfiguration-plugin'
